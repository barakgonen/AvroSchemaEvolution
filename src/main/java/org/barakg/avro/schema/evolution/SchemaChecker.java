...
//    private static CompatibilityChecker checker = CompatibilityChecker.BACKWARD_TRANSITIVE_CHECKER;
    ...
    public static boolean isValidSchemaChange(AvroSchema newSchema, AvroSchema oldSchema) {
...
//                        newSchema.isBackwardCompatible(oldSchema);
    ...
    private static boolean areBothAvroSchemataAreEnums(AvroSchema newSchema, AvroSchema oldSchema) {
        return false;
//        newSchema.
//        return newSchema.rawSchema().getType().equals(Schema.Type.ENUM) &&
//                oldSchema.rawSchema().getType().equals(Schema.Type.ENUM);
    }

    private static boolean validateEnumUniqeCompatability(AvroSchema newSchema, AvroSchema oldSchema) {
//        List<String> newSchemaEnumSymbols = newSchema.rawSchema().getEnumSymbols();
//        List<String> oldSchemaEnumSymbols = oldSchema.rawSchema().getEnumSymbols();
...
//
    ...
    private static boolean isChangeBackwardsCompatible(AvroSchema newSchema, AvroSchema oldSchema) {
//        List<Schema.Field> newSchemaFields = newSchema.rawSchema().getFields();
//        List<Schema.Field> oldSchemaFields = oldSchema.rawSchema().getFields();
//
